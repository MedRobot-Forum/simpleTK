...
//-----------------------------------------------------------------------------
int vtkWidgetMPR::getNumberOfRenderWindow(vtkRenderWindow* t_window) const
{
	for (auto i = 0; i < 3; ++i)
	{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::setWindowLevel(const int& t_window, const int& t_level)
{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::setCameraCentered(const int& t_centered) const
{
	if (m_resliceWidget)
	{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::render(vtkSmartPointer<vtkDICOMReader> reader)
{
	if (!m_mprMaker)
	{
		return;
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::resetResliceWidget()
{
	if (m_resliceWidget)
	{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::changeWindowLevel(vtkRenderWindow* t_renderWindow, const int& t_window,
...
void vtkWidgetMPR::create3DMatrix() const
{
// 	if (m_image)
// 	{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::initializeWidget()
{
...
//-----------------------------------------------------------------------------
void vtkWidgetMPR::createResliceWidget()
{
	if (!m_resliceWidget)
	{
		m_resliceWidget =
...
//-----------------------------------------------------------------------------
//void vtkWidgetMPR::createVTKkWidgetOverlay(vtkRenderWindow* t_window, int& t_windowNumber)
//{
// 	auto* const renderer =
...
//-----------------------------------------------------------------------------
// void vtkWidgetMPR::refreshOverlayInCorner(vtkRenderWindow* t_window, const int t_windowNumber,
// 	const int t_corner)
// {
...
//-----------------------------------------------------------------------------
vtkAxisActor2D* vtkWidgetMPR::createScaleActor(vtkRenderWindow* t_window)
{
